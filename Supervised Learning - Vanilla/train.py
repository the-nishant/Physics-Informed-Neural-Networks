...
xvals = np.array([d[0] for d in dataset])
yvals = np.array([d[1] for d in dataset])
N = 10000
...
class DataPoints(sc.SampleDomain):
    def __init__(self):
        ...
        self.points["x"] = xvals.reshape(-1, 1)
        self.constraints = {
            'y' : yvals.reshape(-1, 1)
        }
        self.points["area"] = np.ones_like(self.points["x"])/N
...
s = sc.Solver(
    sample_domains= (DataPoints(), InteriorInfer()),
    netnodes=[net],
